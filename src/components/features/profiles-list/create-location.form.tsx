...
import { useCreateLocationSchema, type CreateLocationData } from '@/lib/validators/location'
...
export const CreateLocationForm = ({ profileId, onUpdate }: CreateLocationFormProps) => {
  ...
  const handleFormSubmit = async (formData: CreateLocationData) => {
    const { data, error } = await createLocation(formData, profileId)
    if (error) {
